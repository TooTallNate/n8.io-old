...
var url = require('url');
var git = require('./git');
var debug = require('debug')('n8.io:get-sha');
...
module.exports = function (req, res, next) {
  ...
  if (git.OID_HEXSZ != sha.length) {
    ...
    err = git.git_oid_fromstrn.async(short_oid, sha, sha.length);
    if (err !== 0) return next(new Error('git_oid_fromstrn: error ' + rtn));
    ...
    if (err !== 0) return next(new Error('git_commit_lookup_prefix: error ' + rtn));
    commit = commit.deref();
